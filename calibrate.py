...
from simulator import CTMCSEIR, ApproxSEIR, ExactSEIR, Model
...
class LossFunc:
    ...
    def __call__(self, param, steps=100):
        ...
        for i in range(self.n):
            if isinstance(self.model, CTMCSEIR):
                self.model.step()
            else:
...
def plot_model_and_data(model: Model, data):
    fig, ax = plt.subplots()
    t, confirmed, recovered, deaths, cum_conf = data
    ...
    init_state = {'e': confirmed[0],
                  ...
                  'p': deaths[0],
                  'cr': recovered[0]}
    model.reset(state=init_state, keep_init=False)
    for i in range(n):
